...
<div class="max-w-md mx-auto mt-20 mb-32 animate-fade-in">
    <div class="text-center mb-10">
        <a href="{{ route('home') }}" class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-zinc-900 text-white font-serif font-bold text-2xl mb-6 shadow-sm">
            L
        </a>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center gap-2 cursor-pointer group">
                    <input type="checkbox" name="remember" class="w-4 h-4 rounded border-zinc-300 text-accent-600 focus:ring-accent-500 transition-colors cursor-pointer">
                    <span class="text-sm font-medium text-zinc-600 group-hover:text-zinc-900 transition-colors">Remember me</span>
                </label>
            </div>

            <!-- Submit -->
            <button type="submit" class="w-full h-12 bg-zinc-900 text-white rounded-xl font-medium flex items-center justify-center hover:bg-zinc-800 transition-all active:scale-[0.98] shadow-sm">
                Sign in
            </button>
        </form>
